...
public class RunConfig {
	...
	private List<Integer> getTreatments(File fileXHead) {
		...
		String line = "";
		readingStatus flag = readingStatus.SEAK;

		if (fileXHead.exists()) {

			try(Scanner reader=new Scanner(fileXHead)) {

				while (flag != readingStatus.QUIT && reader.hasNextLine()) {
					line = reader.nextLine();
					if (line.contains("*TREATMENTS")) {
						flag = readingStatus.READ;
					}
					if (line.contains("*CULTIVARS")) {
						flag = readingStatus.QUIT;
					}
					if (flag == readingStatus.READ && !line.isEmpty() && !line.startsWith("@") && !line.startsWith("*") && !line.startsWith("!")) {
